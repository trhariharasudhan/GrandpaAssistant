...
from core.commands.handlers.status import handle_status_command
...
def _context() -> CommandContext:
    return CommandContext(
        ...
        offline_mode_status_summary=lambda: "Offline core mode is disabled. Offline summary.",
        offline_help_summary=lambda: "Offline help.",
        offline_ai_status_summary=lambda: "Offline AI status.",
        developer_mode_status_summary=lambda: "Developer mode is disabled. Developer summary.",
        focus_mode_status_summary=lambda: "Focus mode is currently disabled.",
        voice_trainer_status_summary=lambda: "Voice trainer status.",
        piper_setup_summary=lambda: "Piper setup.",
        custom_voice_setup_summary=lambda: "Custom voice setup.",
        custom_voice_license_status_summary=lambda: "Voice license.",
        custom_voice_samples_summary=lambda: "Voice samples.",
        voice_status_summary=lambda: "Voice profile.",
...
class CommandRouterStatusHandlerTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_voice_status_setup_summary_behavior(self) -> None:
        self._run_router("piper setup status", extra_patches=[patch.object(command_router, "_piper_setup_summary", return_value="Piper setup.")])
        self._run_router("voice status", extra_patches=[patch.object(command_router, "voice_status_summary", return_value="Voice profile.")])

        self.assertEqual("Piper setup.", self.spoken[-2])
        self.assertEqual("Voice profile.", self.spoken[-1])

    def test_offline_mode_status_behavior(self) -> None:
        self._run_router(
            "offline mode status",
            extra_patches=[
                patch.object(command_router, "get_setting", side_effect=lambda name, default=None: False if name == "assistant.offline_mode_enabled" else default),
                patch.object(command_router, "_offline_mode_summary", return_value="Offline summary."),
            ],
        )

        self.assertEqual("Offline core mode is disabled. Offline summary.", self.spoken[-1])

    def test_backend_mode_status_summary_behavior(self) -> None:
        self._run_router(
            "developer mode status",
            extra_patches=[
                patch.object(command_router, "get_setting", side_effect=lambda name, default=None: False if name == "assistant.developer_mode_enabled" else default),
                patch.object(command_router, "_developer_mode_summary", return_value="Developer summary."),
            ],
        )

        self.assertEqual("Developer mode is disabled. Developer summary.", self.spoken[-1])

    def test_provider_model_status_behavior(self) -> None:
        self._run_router(
            "offline ai status",
            extra_patches=[patch.object(command_router, "get_setting", side_effect=lambda name, default=None: False if name == "assistant.offline_mode_enabled" else "phi3")],
        )

        self.assertEqual("Offline mode is disabled. Preferred local model is phi3. If the local AI server is unavailable, responses may be limited.", self.spoken[-1])

    def test_unknown_status_like_command_falls_through(self) -> None:
    ...
    def test_mutation_commands_are_not_handled(self) -> None:
        context = _context()
        for command in ["enable offline mode", "disable offline mode", "set piper model path to x", "start listening", "switch to piper voice"]:
            with self.subTest(command=command):
                self.assertFalse(handle_status_command(command, context).handled)
