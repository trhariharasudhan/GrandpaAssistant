...
from core.commands.handlers.contacts import handle_contacts_command
...
def _context() -> CommandContext:
    return CommandContext(
        ...
        local_contacts_summary=lambda: "Local contacts: Priya +91****1234",
        local_contact_lookup_summary=lambda query: f"Found {query} +91****1234.",
        google_contacts_summary=lambda: "Google contacts summary.",
        google_contact_changes_summary=lambda: "Recent contact changes.",
...
class CommandRouterContactsHandlerTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_contact_lookup_summary_behavior(self) -> None:
        self._run_router(
            "find contact Priya",
            extra_patches=[
                patch.object(command_router, "find_contact", return_value={"ok": True, "contact": {"name": "Priya", "phone": "+911234567890"}}),
                patch.object(command_router, "redact_contact_for_display", side_effect=lambda item: {"name": item.get("name"), "phone": "+91****7890"}),
            ],
        )

        self.assertEqual("Found Priya +91****7890.", self.spoken[-1])

    def test_contact_summary_behavior(self) -> None:
        self._run_router(
            "list contacts",
            extra_patches=[patch.object(command_router, "list_contacts", return_value=[{"name": "Priya", "phone": "+91****7890"}])],
        )

        self.assertEqual("Local contacts: Priya +91****7890", self.spoken[-1])

    def test_google_contacts_read_only_summaries(self) -> None:
        self._run_router("list google contacts", extra_patches=[patch.object(command_router, "list_google_contacts", return_value="Google contacts summary.")])
        self._run_router("recent contact changes", extra_patches=[patch.object(command_router, "get_recent_contact_change_summary", return_value="Recent contact changes.")])

        self.assertEqual("Google contacts summary.", self.spoken[-2])
        self.assertEqual("Recent contact changes.", self.spoken[-1])

    def test_contact_alias_and_favorite_summaries(self) -> None:
        self._run_router("list favorite contacts", extra_patches=[patch.object(command_router, "list_favorite_contacts", return_value="Favorite contacts.")])
        self._run_router("list contact aliases", extra_patches=[patch.object(command_router, "list_contact_aliases", return_value="Contact aliases.")])

        self.assertEqual("Favorite contacts.", self.spoken[-2])
        self.assertEqual("Contact aliases.", self.spoken[-1])

    def test_unknown_contact_like_command_falls_through(self) -> None:
    ...
    def test_mutation_and_action_commands_are_not_handled(self) -> None:
        context = _context()
        for command in [
            "call Priya",
            "message Priya saying hello",
            "email Priya report",
            "add contact Priya 1234567890",
            "delete contact Priya",
            "sync google contacts",
            "refresh google contacts",
            "favorite contact Priya",
            "set contact alias amma to Priya",
            "what is Priya phone",
            "copy Priya phone",
        ]:
            with self.subTest(command=command):
                self.assertFalse(handle_contacts_command(command, context).handled)
