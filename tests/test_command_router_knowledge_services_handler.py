...
from core.commands.handlers.knowledge_services import handle_knowledge_services_command
...
def _context() -> CommandContext:
    return CommandContext(
        ...
        language_mode_status_summary=lambda: "Language mode is auto.",
        ...
        knowledge_review_queue_summary=lambda: "Knowledge review queue is empty.",
...
class CommandRouterKnowledgeServicesHandlerTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_language_status_summary_behavior(self) -> None:
        self._run_router("language mode status", extra_patches=[patch.object(command_router, "language_mode_status", return_value="Language mode is auto.")])

        self.assertEqual("Language mode is auto.", self.spoken[-1])

    def test_meeting_summary_behavior(self) -> None:
        self._run_router("meeting summary", extra_patches=[patch.object(command_router, "meeting_mode_summary", return_value="Meeting summary.")])

        self.assertEqual("Meeting summary.", self.spoken[-1])

    def test_rag_and_local_knowledge_status_behavior(self) -> None:
        ...
        self.assertEqual("RAG library.", self.spoken[-2])
        self.assertEqual("Knowledge review queue is empty.", self.spoken[-1])

    def test_proactive_suggestion_summary_behavior(self) -> None:
        self._run_router("proactive suggestions", extra_patches=[patch.object(command_router, "_proactive_suggestions_summary", return_value="Proactive suggestions.")])

        self.assertEqual("Proactive suggestions.", self.spoken[-1])

    def test_learning_status_summary_behavior(self) -> None:
        self._run_router("learning status", extra_patches=[patch.object(command_router, "_learning_status_summary", return_value="Learning status.")])

        self.assertEqual("Learning status.", self.spoken[-1])

    def test_unknown_knowledge_service_command_falls_through(self) -> None:
        ask = Mock(return_value="Existing fallback reply.")
        self._run_router(
            "knowledge service banana mode",
            extra_patches=[
    ...
    def test_mutation_commands_are_not_handled(self) -> None:
        context = _context()
        for command in [
            "set language mode tamil",
            "capture meeting note hello",
            ...
            with self.subTest(command=command):
                self.assertFalse(handle_knowledge_services_command(command, context).handled)
