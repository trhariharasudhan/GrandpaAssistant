...
from core.commands.handlers.device_status import handle_device_status_command
...
def _context() -> CommandContext:
    return CommandContext(
        ...
        phone_link_status_summary=lambda: "Phone link ready.",
        face_security_status_summary=lambda: "Face security status.",
        startup_auto_launch_status_summary=lambda: "Startup auto launch status.",
        smart_home_setup_summary=lambda: "Smart Home setup help.",
...
class CommandRouterDeviceStatusHandlerTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_phone_link_readiness_status_behavior(self) -> None:
        self._run_router("phone link status", extra_patches=[patch.object(command_router, "_phone_link_status_summary", return_value="Phone link ready.")])

        self.assertEqual("Phone link ready.", self.spoken[-1])

    def test_face_security_enrollment_status_behavior(self) -> None:
        self._run_router("face security status", extra_patches=[patch.object(command_router, "_face_security_status_summary", return_value="Face security status.")])

        self.assertEqual("Face security status.", self.spoken[-1])

    def test_startup_auto_launch_status_behavior(self) -> None:
        self._run_router("auto launch status", extra_patches=[patch.object(command_router, "startup_auto_launch_status", return_value="Startup auto launch status.")])

        self.assertEqual("Startup auto launch status.", self.spoken[-1])

    def test_unknown_device_status_command_falls_through(self) -> None:
        ask = Mock(return_value="Existing fallback reply.")
        self._run_router(
            "device readiness banana mode",
            extra_patches=[
    ...
    def test_mutation_and_control_commands_are_not_handled(self) -> None:
        context = _context()
        for command in [
            "enroll my face",
            "verify my face",
            ...
            with self.subTest(command=command):
                self.assertFalse(handle_device_status_command(command, context).handled)
