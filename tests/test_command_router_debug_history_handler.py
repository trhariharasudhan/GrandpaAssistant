...
class CommandRouterDebugHistoryHandlerTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def _handler(self, command: str, **kwargs):
        return handle_debug_command(
            ...
            debug_timeline_summary=kwargs.get("debug_timeline_summary", lambda: "Debug timeline."),
            debug_search_summary=kwargs.get("debug_search_summary", lambda query: f"Debug search {query}."),
            reuse_suggestions_summary=kwargs.get("reuse_suggestions_summary", lambda language="auto": f"Reuse {language}."),
            debug_learning_summary=kwargs.get("debug_learning_summary", lambda: "Debug learning."),
            debug_checklist_summary=kwargs.get("debug_checklist_summary", lambda language="auto": f"Checklist {language}."),
            allow_session_dashboard=kwargs.get("allow_session_dashboard", True),
            allow_fix_read_only=kwargs.get("allow_fix_read_only", True),
            allow_history=kwargs.get("allow_history", True),
        )
    ...
    def test_debug_timeline_behavior_unchanged(self) -> None:
        self._run_router("debug timeline", extra_patches=[patch.object(command_router, "_debug_timeline_summary", return_value="Debug timeline.")])

        self.assertEqual("Debug timeline.", self.spoken[-1])

    def test_debug_session_search_behavior_unchanged(self) -> None:
        self._run_router(
            "search debug sessions for startup",
            extra_patches=[patch.object(command_router, "_debug_search_summary", return_value="Debug search startup.")],
        )

        self.assertEqual("Debug search startup.", self.spoken[-1])

    def test_reuse_suggestions_behavior_unchanged(self) -> None:
        self._run_router(
            "seen this before",
            extra_patches=[patch.object(command_router, "_debug_reuse_suggestions_summary", return_value="Reuse auto.")],
        )

        self.assertEqual("Reuse auto.", self.spoken[-1])

    def test_debug_learning_summary_behavior_unchanged(self) -> None:
        self._run_router(
            "debug learning summary",
            extra_patches=[patch.object(command_router, "_debug_learning_summary", return_value="Debug learning.")],
        )

        self.assertEqual("Debug learning.", self.spoken[-1])

    def test_debug_checklist_behavior_unchanged(self) -> None:
        self._run_router(
            "issue varama check pannu",
            extra_patches=[patch.object(command_router, "_debug_checklist_summary", return_value="Checklist ta.")],
        )

        self.assertEqual("Checklist ta.", self.spoken[-1])

    def test_unknown_debug_history_command_falls_through(self) -> None:
        ask = Mock(return_value="Existing fallback reply.")
        self._run_router(
            "debug history banana",
            extra_patches=[
    ...
    def test_dangerous_fix_action_commands_are_not_handled(self) -> None:
        for command in ["apply fix", "dismiss fix abc123", "allow abc123", "execute fix", "rollback fix"]:
